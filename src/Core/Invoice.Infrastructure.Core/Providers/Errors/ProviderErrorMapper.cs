...
public static class ProviderErrorMapper
{
    ...
    public static string MaskErrorMessage(string? message, string providerKey)
    {
        ...
        var maskedMessage = MaskingHelper.MaskSecret(message);
        maskedMessage = MaskingHelper.MaskVkn(maskedMessage);
        maskedMessage = MaskingHelper.MaskPhone(maskedMessage);
