        ...
        public async Task<ProviderSendResult> SendInvoiceAsync(InvoiceEnvelope envelope, ProviderConfig config, CancellationToken ct = default)
        {
            ...
            if (string.IsNullOrWhiteSpace(config?.ApiBaseUrl))
            {
                return ProviderSendResult.Failed(
                    provider: ProviderType.IDEA,
                    errorCode: "CONFIG_MISSING",
                    errorMessage: "İdea için ApiBaseUrl tanımlı değil. Admin → ProviderConfig üzerinden ekleyiniz."
                );
            }
            ...
            try
            {
                ...
                var requestUri = $"{baseUrl}/api/efatura/send";
                ...
                if (!resp.IsSuccessStatusCode)
                {
                    ...
                    return ProviderSendResult.Failed(
                        ...
                        errorMessage: $"İdea servis hatası: {(int)resp.StatusCode} {resp.ReasonPhrase}. Yanıt: {body}"
                    );
                }
                ...
                return ProviderSendResult.SuccessResult(
                    ...
                    providerResponseMessage: "İdea: e-Fatura isteği başarıyla iletildi.",
                    ublXml: null
                );
            }
            catch (HttpRequestException ex)
            {
                ...
                return ProviderSendResult.Failed(
                    ...
                    errorMessage: $"İdea HTTP hatası: {ex.Message}"
                );
            }
            catch (Exception ex)
            {
                return ProviderSendResult.Failed(
                    ...
                    errorMessage: $"İdea genel hata: {ex.Message}"
                );
            }
        }
