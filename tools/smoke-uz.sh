...
echo "== UZ Sandbox Test v2 başlıyor =="
...
if [[ -z "$INVOICE_FILE" ]]; then
    ...
    echo "Örnek: $0 output/TEST_INVOICES/UZ_MULTI_ITEMS.json"
    echo ""
    echo "Mevcut test dosyaları:"
    ls -1 output/TEST_INVOICES/UZ_*.json 2>/dev/null || echo "Henüz test dosyası yok"
    exit 1
fi
...
# Konfigürasyon
UZ_BASE_URL="https://sandbox.didox.uz"
AUTH_ENDPOINT="/api/auth/e-imzo"
INVOICE_ENDPOINT="/api/invoices"

# Test credentials (appsettings'den okunmalı)
CLIENT_ID="SANDBOX_UZ_CLIENT_ID"
CLIENT_SECRET="SANDBOX_UZ_CLIENT_SECRET"
CERT_PATH="certs/sandbox_uz.p12"
CERT_PASS="SANDBOX_UZ_CERT_PASS"
PINFL="123456789"

log_info "UZ Sandbox test başlatılıyor..."
log_info "Base URL: $UZ_BASE_URL"
log_info "Invoice File: $INVOICE_FILE"

echo "== Step 1: UZ Sandbox Auth Test =="
echo "Endpoint: POST $UZ_BASE_URL$AUTH_ENDPOINT"
echo "Status: Sandbox endpoint'e bağlanıyor..."

# E-IMZO signature oluştur (simulated)
log_info "E-IMZO signature oluşturuluyor (simulated)..."
EIMZO_SIGNATURE="base64_encoded_signature_$(date +%s)"

# Auth request with retry
log_info "Auth endpoint'e bağlanılıyor..."

    AUTH_RESPONSE=$(curl -s -w "\n%{http_code}" \
      -X POST "$UZ_BASE_URL$AUTH_ENDPOINT" \
      -H "Content-Type: application/json" \
      -d "{
        \"pinfl\": \"$PINFL\",
        \"eimzoSignature\": \"$EIMZO_SIGNATURE\",
        \"clientId\": \"$CLIENT_ID\",
...
if [[ "$HTTP_CODE" == "200" ]]; then
    ...
    if [[ -n "$TOKEN" && "$TOKEN" != "null" ]]; then
        ...
        echo "$RESPONSE_BODY" > output/responses/UZ_AUTH_RESPONSE.json
        log_info "Token kaydedildi: output/responses/UZ_AUTH_RESPONSE.json"
    ...
    echo "$RESPONSE_BODY" > output/responses/UZ_AUTH_RESPONSE.json
    log_info "Response kaydedildi: output/responses/UZ_AUTH_RESPONSE.json"
    
    # Error logging
    if [[ "$HTTP_CODE" != "200" ]]; then
        log_error "UZ Auth Error - HTTP $HTTP_CODE" >> logs/sandbox-errors.log
        echo "Response: $RESPONSE_BODY" >> logs/sandbox-errors.log
    fi
fi

echo "== Step 2: Invoice Endpoint Test =="
echo "Endpoint: POST $UZ_BASE_URL$INVOICE_ENDPOINT"
...
if [[ -n "$TOKEN" && "$TOKEN" != "null" ]]; then
    ...
    INVOICE_RESPONSE=$(retry_with_backoff "$SANDBOX_MAX_RETRIES" "$SANDBOX_DELAY" "$SANDBOX_MULTIPLIER" \
      'curl -s -w "\n%{http_code}" \
        -X POST "$UZ_BASE_URL$INVOICE_ENDPOINT" \
        -H "Authorization: Bearer $TOKEN" \
        -H "Content-Type: application/json" \
        -d "@$INVOICE_FILE" 2>/dev/null || echo "CURL_ERROR"')
    ...
    if http_check "$INVOICE_HTTP_CODE"; then
        ...
        BASE_NAME=$(basename "$INVOICE_FILE" .json)
        RESPONSE_FILE="output/responses/SANDBOX_UZ_INVOICE_RESPONSE_${BASE_NAME}_$(date +%Y%m%d_%H%M%S).json"
        ...
        BASE_NAME=$(basename "$INVOICE_FILE" .json)
        ERROR_FILE="output/responses/SANDBOX_UZ_INVOICE_ERROR_${BASE_NAME}_$(date +%Y%m%d_%H%M%S).json"
        echo "$INVOICE_RESPONSE_BODY" > "$ERROR_FILE"
        log_info "Error response kaydedildi: $ERROR_FILE"
        
        # Error logging
        log_error "UZ Invoice Error - HTTP $INVOICE_HTTP_CODE" >> logs/sandbox-errors.log
...
echo "✅ UZ Sandbox test tamamlandı"
echo "📁 Invoice File: $INVOICE_FILE"
...
log_success "UZ Sandbox test tamamlandı - $INVOICE_FILE"
